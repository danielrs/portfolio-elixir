defmodule PortfolioWeb.API.V1.UserController do
  ...
  defp authorize_creation(conn, _params) do
    ...
    if user && user.role.admin? do
      ...
      |> render(PortfolioWeb.API.V1.SessionView, "forbidden.json", error: "You are not authorized to create new users!")
      |> halt
    end
  end
  ...
  defp authorize_modification(conn, _opts) do
    ...
    if user && (user.role.admin? or user.id == id) do
      ...
      conn
      |> put_status(:forbidden)
      |> render(PortfolioWeb.API.V1.SessionView, "forbidden.json", error: "You are not authorized to modify that user!")
      |> halt
    end
  end
end
